...
const getSessionUser = async () => {
  try {
    ...
    sessionUser.value = res.data.message
    console.log('Session user:', sessionUser.value)
...
const fetchBookings = async (user) => {
  try {
    const res = await api.get('/Room Reservation', {
      params: {
        filters: JSON.stringify([['booked_by', '=', user]]),
        fields: JSON.stringify(['subject', 'date', 'from_time', 'to_time', 'room', 'booked_by']),
